...
def main():
    ...
    if args.before:
        ...
        nb_leaf, nb_code = len(leaves(before)), sum(1 for n in _all(before) if n.code)
        print('')
        print('■ 元のツリーとの違い')
        if nb_leaf != n_leaf or nb_code != n_code:
            print('  ! 最終ラベル %d個→%d個 / コード %d個→%d個 と数が変わっています。'
                  '検査で行が消えていないか確認してください' % (nb_leaf, n_leaf, nb_code, n_code))
